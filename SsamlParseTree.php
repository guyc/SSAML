...
class SsamlParseTree
{
     ...
     * tree with a SsamlParseTree for each line, and
     * all child-lines stored as an array of SsamlParseTree's
     * in $tree->children[]
     */
    static function Parse($Text)
    {
        ...
        $tree = new SsamlParseTree();
        $tree->indent = -1;  // unbeatable indent!
        $stack = array($tree);
        $top = $tree;
        foreach (SsamlParseTree::ParseLines($Text) as $line) {
    ...
    /*
     * Generates a SsamlDoc from the parse tree.
     * Returns a SsamlDoc().  Walks the SsamlParseTree
     * and generates a corresponding tree of SsamlNode 
     * objects.
     */
    function Generate()
    {
        // The top node of the tree is distinguised by having $text==null
        // and that causes it to be a SsamlDoc which is really just a container
        // for the child nodes.
        if ($this->text === null) {
            $node = new SsamlDoc();
        } else {
            ...
            foreach (SsamlParser::$NODE_CLASSES as $nodeClass) {
